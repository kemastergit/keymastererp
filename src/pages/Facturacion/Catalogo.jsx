...
export default function Catalogo({
    ...
    const [viewMode, setViewMode] = useState(() => localStorage.getItem('catalogo_view_mode') || 'list');
    ...
    useEffect(() => {
        if (selectedIndex >= 0 && listRef.current) {
            let selectedEl;
            if (viewMode === 'grid') {
                selectedEl = listRef.current.children[0]?.children[selectedIndex]
            } else {
                selectedEl = listRef.current.children[0]?.children[selectedIndex]
            }
            if (selectedEl) {
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex bg-[var(--surfaceDark)] p-1 rounded-lg border border-[var(--border-var)] gap-1">
                        <button 
                            ...
                            className={`w-7 h-7 flex items-center justify-center rounded-md transition-all ${viewMode === 'grid' ? 'bg-[var(--teal)] text-white shadow-sm' : 'text-[var(--text2)] hover:bg-[var(--surface)]'}`}
                        ...
                        <button 
                            ...
                            className={`w-7 h-7 flex items-center justify-center rounded-md transition-all ${viewMode === 'list' ? 'bg-[var(--teal)] text-white shadow-sm' : 'text-[var(--text2)] hover:bg-[var(--surface)]'}`}
                            title="Vista Lista"
                        >
                    ...
                    <div className="relative flex-1">
                        <input 
                            ...
                            onKeyDown={e => {
                                ...
                                // Navegación inteligente según vista
                                if (e.key === 'ArrowDown') {
                                    e.preventDefault();
                                    ...
                                    // En grid permitimos horizontal también
                        ...
                        {articulos.map((a, index) => {
                            ...
                            if (viewMode === 'grid') {
                                return (
                                    <div 
                                        key={a.id}
                                        onClick={() => { if (hasStock) addToCart(a); setSelectedIndex(index); }}
                                        className={`
                                    ...
                                    >
                                        ...
                                        {q > 0 && <div className="absolute top-1.5 right-1.5 bg-[var(--green-var)] text-white w-5 h-5 rounded-full flex items-center justify-center shadow-md font-black text-[9px] border border-[var(--surface)]">{q}</div>}
                                    </div>
                                )
                            }

                            // VISTA LISTA (LÍNEAS) - DISEÑO PREMIUM
                            return (
                                <div 
                                    key={a.id}
                                    onClick={() => { if (hasStock) addToCart(a); setSelectedIndex(index); }}
                                    ...
                                    className={`
                                        ...
                                        ${isSelected 
                                            ? 'bg-white border-[var(--teal)] shadow-[0_10px_15px_-3px_rgba(20,184,166,0.1)] scale-[1.01] z-10' 
                                            : index % 2 === 0 ? 'bg-white border-transparent' : 'bg-slate-50 border-transparent'}
                                        ${!hasStock ? 'opacity-60 grayscale-[0.8]' : 'hover:bg-white hover:border-[var(--teal)]/30 hover:shadow-sm'}
                                    `}
                                >
                                    {/* Indicador lateral para el seleccionado */}
                                    {isSelected && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-3/5 bg-[var(--teal)] rounded-r-full"></div>}
                                    ...
                                    <div className="w-32 flex items-center justify-end gap-1.5 pl-2">
                                        {q > 0 ? (
                                            <div className="flex items-center bg-white border border-slate-200 rounded-xl p-1 shadow-sm ring-1 ring-black/5 animate-in fade-in zoom-in duration-200">
                                                ...
                                                <div className="w-8 text-center font-black text-[12px] text-[var(--teal)]">
                                                    {q}
                                                </div>
                                                <button 
                                                    className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-[var(--teal)] hover:text-white flex items-center justify-center transition-colors active:scale-90"
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        addToCart(a);
                                                    }}
                                            ...
                                            <button 
                                                className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all shadow-sm active:scale-90
                                                    ${hasStock ? 'bg-emerald-50 text-[var(--teal)] hover:bg-[var(--teal)] hover:text-white' : 'bg-slate-100 text-slate-300'}`}
                                                disabled={!hasStock}
                                            >
